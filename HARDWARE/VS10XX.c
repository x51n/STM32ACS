...
{								 
	SPI2_SetSpeed(SPI_BaudRatePrescaler_64);//设置到低速模式 
}
//SD卡正常工作的时候,可以高速了
...
void VS_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;				 //PB12 XCS
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //输入
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOB, &GPIO_InitStructure);

 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_4;//PF6,PF7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_Init(GPIOA, &GPIO_InitStructure);	
	
	GPIO_SetBits(GPIOA,GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_4);
